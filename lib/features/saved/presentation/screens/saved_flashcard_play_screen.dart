...
class SavedFlashcardPlayScreen extends StatefulWidget {
  const SavedFlashcardPlayScreen({
    super.key,
  ...
  @override
  State<SavedFlashcardPlayScreen> createState() =>
      _SavedFlashcardPlayScreenState();
}

class _SavedFlashcardPlayScreenState extends State<SavedFlashcardPlayScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final detail = await _repository.getSavedFlashcardSetDetail(
        widget.savedListItemId,
      );
  ...
  Future<void> _applyDetail(SavedLibraryDetail detail) async {
    ...
    _createdAt ??= _readCreatedAt(detail.raw) ?? widget.createdAt;

    _toolData = buildFlashcardToolDataFromSavedDetail(
      detail.raw,
      fallbackTitle: widget.title,
    );
  }

  DateTime? _readCreatedAt(Map<String, dynamic> raw) {
    final direct = raw['createdAt'] ?? raw['savedAt'];
    if (direct != null) return DateTime.tryParse(direct.toString());
    final set = raw['flashcardSet'] ?? raw['flashcard_set'] ?? raw['set'];
    if (set is Map) {
      final nested = set['createdAt'] ?? set['savedAt'];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: MishkaAppBar(
        title: l10n.savedFlashCardTitle,
        topTitle: l10n.saved,
        showBack: true,
        showBottomBar: true,
      ),
